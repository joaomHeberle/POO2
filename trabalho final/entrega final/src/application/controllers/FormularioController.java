...
public class FormularioController implements Initializable {
	...
	void handleAlteraJogador(ActionEvent event) {
	 ...
	 @FXML
	    ...
	    void handleEscolheMissao(ActionEvent event) {
		  Jogador alvo = this.tabJogador.getSelectionModel().getSelectedItem();
			if (alvo != null) {
				...
				Main.loadScene("EscolheMissao", alvo);
	  ...
	  @FXML
	    void handleListaMissoes(ActionEvent event) {
		
		  Main.loadScene("ListaMissao");
	    }
	  
	    @FXML
	    void handleVerificarMissoes(ActionEvent event) {
	    	Jogador alvo = this.tabJogador.getSelectionModel().getSelectedItem();
	    	  if (alvo != null) {
	    	
	    			 Main.loadScene("ListaMissaoJogador",alvo);
	    			 	
				} else {
					var alert = new Alert(AlertType.INFORMATION);
					alert.setHeaderText("Jogador não selecionado");
					alert.setContentText("selecione para poder ver suas missões");
					alert.setTitle("Atenção");
